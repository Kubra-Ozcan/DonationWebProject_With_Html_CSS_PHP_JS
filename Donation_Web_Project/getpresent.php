...
if (isset($_POST["present"])) {
    ...
    {
        $agree_err = "Email alanı boş bırakılamaz.";
    } 
    else 
    {
        $agree = $_POST["agree"];
    ...
    if (isset($name)  && isset($lastname) && isset($image) && isset($password) && isset($email )&& isset($phone) && isset($address) )
    {
        ...
        $ekle = "INSERT INTO present (FirstName,LastName,image,password,Email,PhoneNum,Address) values ('$name','$lastname','$imgContent',
        ...
        if ($calistirekle) 
        {
            echo '<div class="alert alert-success" role="alert" style="font-size:80px; , text-align:center;,  height:4px;,
            width: 83px;, text-color:gold;">
            Receiving form  added successfully. 
            </div>';
...
<html>
   ...
   <head>
      <title>Form for the Get Present</title>
   ...
   <body bgcolor = "#FFFFFF">
            ...
            <form action="getpresent.php" method = "post" enctype="multipart/form-data">
            <div class="wrapper">
                    <div class="title">Receive   Present
                    </div>
                    <div class="form">
                    ...
                    <div class="inputfield">
                        <input type="submit" value="Apply for the Present"  name="present" class="btn">
                    </div>
                    </div>
                </div>
